...
def deleteUserDocuments(get_response):
    def middleware(request):
        ...
        for p in uploaded_DocDetails.objects.raw('SELECT * from homePageApp_uploaded_docdetails where uploadedTime < now() - interval 6 hour'):
        ...
        cursor.execute('''DELETE from homePageApp_uploaded_docdetails where uploadedTime < now() - interval 6 hour''') # removes tuple from DB itself
